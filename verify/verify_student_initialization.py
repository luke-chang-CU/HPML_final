...
def verify_student_initialization():
    ...
    start_tokens = selected_tokens[:, 0:1].to(device)
    
    # 2. Load Student (10L_1024E)
    print("Loading Student (10L_1024E)...")
    config = GPTConfig(
        vocab_size=1024,
        block_size=256,
        n_layer=10, 
    ...
    # Using Final Epoch
    ckpt_path = 'checkpoints_distill_hybrid/student_10L_1024E_final.pt'
    ...
    for i in range(5):
        ...
        # B. Decode Original
        idx_real = selected_tokens[i].view(1, 256)
        ...
        save_path = os.path.join(output_dir, f'student_comparison_{i}.png')
        img_pil.save(save_path)
        print(f"Saved {save_path} (Left: Gen, Right: Real)")

if __name__ == "__main__":
    verify_student_initialization()
